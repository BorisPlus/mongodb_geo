...
Результат исследования представляет собой инструмент отображения объектов данных типов, сведения о которых хранятся в MongoDB, на карте посредством web-доступа. Клиентская часть реализована с использованием [Leaflet](https://leafletjs.com/) (JavaScript-библиотека с открытым исходным кодом для мобильных интерактивных карт) и набора соотвествующих процедур асинхронного получения сведений от серверной части. Сервис реализован на базе созданного ранее на курсе ["OTUS.Web-python"](https://otus.ru/lessons/webpython/) конструктора программного обеспечения ["Dummy"](https://pypi.org/project/dummy_wsgi_framework/)([репозиторий](https://github.com/BorisPlus/dummy_wsgi_framework)) на языке программирования Python3 с задействованием WSGI. Особенности реализации процедур сбора демонстрационных сведений отражены по мере изложения, но основное внимание уделено простоте работы с геоинформацией средствами MongoDB.
...
Представляемое в рамках настоящей работы решение предполагает возможность хранения в обособленных коллекциях одной базы сведений о гео-объектах различных типов. Тем не менее хранение также возможно и в рамках одной коллекции. Названия полей, описывающих гео-объекты, могут быть произвольными, за исключением уникально идентифицирующего атрибута документа - `ident`. Также необходимо соблюдать установленную MongoDB [структуру сведений](https://docs.mongodb.com/manual/geospatial-queries/#geojson-objects) о гео-объекте:
...
Кроме этого в наборе имеется 7315 метеоритов, чье местоположение не известно. Данный факт также не позволит включить их в гео-индекс и приведет к ошибке. Поэтому в индексе учтены только те метеориты, чье местоположение известно (38400 штук).
...
Изначально сведения о местоположении полигона разнесены по полям записи, то есть фактически отсутствует атрибут необходимой структуры для гео-объекта типа "полигон". Поэтому в его качестве выступает дополнительное поле `area`, куда перенесены сведения в необходимом формате. Структура гео-типа `полигон` имеет вид: 
...
[Сбор](./foreign/onetime_static_load_polygons_wikimapia.py) демонстрационных данных реализован на языке программирования Python3 с использованием библиотеки `pymongo`, поддерживающей исполнение набора инструкций вставки-обновления (`UPSERT`) за раз (`bulk_write(instructions)`).
...
Изначально сведения о маршруте не имеют необходимой структуры для гео-объекта типа "линия". Поэтому в качестве данного атрибута выступает дополнительное поле `area`, куда перенесены сведения в необходимом формате. Структура гео-типа `линия` имеет вид: 
```
...
Использование иной операции [$nearSphere](https://docs.mongodb.com/manual/reference/operator/query/nearSphere) продемонстрировано на примере выборки из MongoDb полигонов и метеоритов с целью отображения их на карте только при условии присутствия (для полигоноа - пересечения) их в "круговой" окрестности наблюдения. 
...
> Операции `$near` и `$nearSphere` шире по возможности, чем просто "нахождение в круговой окрестности", так как описывают не только минимальное удаление (`$maxDistance`) от точки наблюдения, но и минимальное (`$minDistance`). Данное обстоятельство может быть использовано при работе с секторами, учитывающими "примерное" удаление объектов от исходной точки наблюдения: углы обзора видеокамер городского наружного наблюдения, "классические" области действия сотовых вышек и иное.

## Сервис демонстрации гео-объектов

Сервис реализован на базе авторского конструктора программного Web-обеспечения ["Dummy WSGI Framework"](https://pypi.org/project/dummy_wsgi_framework/), является легковесным (45 килобайт кода).
...
При перемещении в определенные районы (достаточно в исходной точке г. Санкт-Петербурга уменьшить масштаб до размера "4") подгружаются указатели на конкретные места падения метеоритов. 
...
Очередным открытием в имеющихся в NASA данных явилось наличие 64 метеоритов, местоположение падения которых имеют (абсолютно) одинаковые координаты. Указанное обнаружено в результате визуального изучения метеоритов на карте (выделяющаяся темная тень).
...
Исходные сведения о перемещении таксопарка "Яндекс" обезличены и содержат крайние точки маршрута следования:
...
> Формат предположительно свидетельствует о том, что система "Яндекс" ориентирована на хранение ненормализованного вида сведений с целью повышения скорости предоставления крайних позиций автомобилей.

На их основании сформированы линия перемещения и фактическая крайняя точка местопребывания в необходимом формате:
...
**Важное замечание**: с целью использования `near` и `nearSphere` необходимо создать индекс `2d` или `2dsphere`, иначе их вызов приведет к (высоко информативной, кстати) ошибке исполнения:
...
Таким образом, при необходимости отображения на карте сведений из иной коллекции необходимо определиться с их видом (статичный, динамичный), типом (точка, линия, полигон) и названием конкретного атрибуда, где они сохранены в требуемой MongoDB структуре. 

При отсутствии в конфигурации динамичной коллекции, генерируемая сервисом HTML-страница не содержит инструкцию на JavaScript, осуществляющую переодичный запрос сведений при неизменной локации, что снижает нагрузку на серверную часть сервиса.
...
Получение сведений клиентской частью от серверной реализовано с использованием асинхронных запросов (AJAX). При этом с целью недопущения исполнения сервисом запросов, являющихся промежуточными, запросы происходят с некоторой малой задержкой, во время которой данный запрос может быть отменен последующим обращением. То есть результативная выборка данных происходит только в конечном положении наблюдения. Данное подход снижает нагрузку на серверную часть за счет исключения к ней AJAX-запросов при последовательном переходе от участка карты к участку и при работе с ее масштабированием.
...
Работа по старту AJAX-запроса и его прерыванию, если пользователь осуществил новое обращение, продемонстрирована ниже. 

![CLEVER_AJAX](./README.files/7_CLEVER_AJAX.gif). 

Промежуточные перемещения и зуммирования не приводят к совершению каких-либо действий серверной стороной.
...
В рамках работы не рассматривались вопросы шардирования сведений о гео-объектах (операции их [поддерживают в MongoDB](https://docs.mongodb.com/manual/geospatial-queries/#geospatial-indexes-and-sharded-collections) в версии выше 4.0) и не исследовалась отдача при нагрузочном тестировании. 

### Расширение источников

В результате рефакторинга представляется возможной организация запросов сведений не только из разных коллекций, как это реализовано в настоящее время, но и от разных баз данных MongoDB. Указанное значительно повысит гибкость использования разработанного инструмента.

## Выводы

Необходимо заметить, что Leaflet через браузер позволяет осуществить запрос у пользователя его текущих координат. Таким образом, представленный результат позволяет организовать на его базе ядро практически любого онлайн-сервиса - от работы с привычными статичными объектами (банкоматы, столовые, места общего пользования, места подзарядки телефонов, остановки городского транспорта) до мониторинга динамичных (друзья рядом, грузо-пассажирский [транспорт](https://www.flightradar24.com/), погодный (температура, осадки) фронт). 
