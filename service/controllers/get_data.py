...
def get_response(environ, start_response, app_config,
                 geo_object_type, north_east_lat, north_east_lon, south_west_lat, south_west_lon):
    """
    ...
    :param app_config:
    :param geo_object_type: dynamic, static, all - geo object type
    ...
    variants = tuple()
    if geo_object_type not in (DYNAMIC, STATIC, 'all'):
        geo_object_type = STATIC
    if geo_object_type == DYNAMIC:
        variants = (DYNAMIC,)
    if geo_object_type == STATIC:
        variants = (STATIC,)
    if geo_object_type == 'all':
    ...
    with pymongo.MongoClient(MONGODB_CONNECTION_STRING) as client:
        for geo_object_type in filter(lambda x: x in variants, MONGODB_DB_COLLECTIONS):
